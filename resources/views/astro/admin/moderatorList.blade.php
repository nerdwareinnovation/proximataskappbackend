...
@section('content')
    <div class="row layout-top-spacing">
        <div class="col-md-12" style="text-align: right">
            <a href="{{route('admin.addModerator')}}" > <button class="btn btn-primary">Add New Moderator</button></a>
        ...
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing mt-3">
            ...
            <div class="widget-content widget-content-area br-6">
                <div class="offset-2">
                    <form method="POST" enctype="multipart/form-data" action="{{route('admin.filterModerator')}}">@csrf
                        <div class="row">
                    ...
                    <table id="zero-config" class="table table-hover" style="width:100%">
                        ...
                        @foreach($moderators as $moderator)
                            <tr>
                                <td>
                                    ...
                                    @if($moderator->isOnline())
                                        ...
                                        <div class="avatar avatar-xl avatar-indicators avatar-online">
                                            @if($moderator->moderatorDetails->image_url)
                                                <img alt="avatar" src="{{asset($moderator->moderatorDetails->image_url)}}" class="rounded-circle" />
                                            @else
                                ...
                                <td><a  onclick="moderatorKPI({{$moderator->id}})">{{$moderator->name}}</a></td>
                                <td>@if(isset($moderator->moderatorDetails->astro_name)){{$moderator->moderatorDetails->astro_name}}@else NOT SET @endif</td>
                                <td>{{$moderator->email}}</td>
                                <td>{{$moderator->moderatorDetails->address}}</td>

                                <td>{{$moderator->moderatorDetails->total_question_answered}}</td>
                                <td>
                                    <a href="{{URL::to('admin/editModerator/'.$moderator->id)}}" >
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 text-success"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>

                                    </a>
                                @if($moderator->status == 1)
                                    <a href="{{URL::to('admin/user/disable/'.$moderator->id)}}" id="disable">
                                ...
                                @else
                                    <a href="{{URL::to('admin/user/active/'.$moderator->id)}}" id="enable">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                                    </a>
                                @endif
                                    <a href="{{URL::to('admin/user/delete/'.$moderator->id)}}" id="delete">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Moderator KPI</h5>
...
@section('pagespecificscripts')
    ...
    <script>
        function moderatorKPI(id){

            $.ajax({
                url: "/admin/moderatorKPI/"+id,
                type:"GET",
                // data:{
